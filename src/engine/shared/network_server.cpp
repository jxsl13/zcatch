...
/**
 * @brief Consists of 4 characters, converted to an int.
 * @details [long description]
 *
 * @param char [description]
 * @return an integer security token
 */
...
bool CNetServer::Open(NETADDR BindAddr, CNetBan *pNetBan, int MaxClients, int MaxClientsPerIP, int Flags)
{
	...
	// anti spoof stuff
	m_NumConAttempts = 0; // logs flooding attacks
	m_TimeNumConAttempts = time_get();
	// fills seed with random stuff
...
SECURITY_TOKEN CNetServer::GetToken(const NETADDR &Addr)
{
	...
	// digest is the final md5 hash value
...
/**
 * @brief check how many clients have the same address
 */
...
void CNetServer::OnTokenCtrlMsg(NETADDR &Addr, int ControlMsg, const CNetPacketConstruct &Packet)
{
	...
	else if (ControlMsg == NET_CTRLMSG_ACCEPT)
	{
		...
		if (Token == GetToken(Addr))
		{
			...
			if (g_Config.m_Debug)
				dbg_msg("security", "correct security token");
